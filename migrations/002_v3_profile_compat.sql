-- Compatibility hotfix for V3 profile consumption from backend services.
-- 1) Prevent survey state truncation.
-- 2) Expose a flat profile view consumed by onboarding/orchestrator.
...
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'public'
          ...
          AND column_name = 'awaiting_question_code'
          AND character_maximum_length IS NOT NULL
          AND character_maximum_length < 50
    ) THEN
        ALTER TABLE conversation_state
            ALTER COLUMN awaiting_question_code TYPE varchar(50);
    END IF;
END $$;

DROP VIEW IF EXISTS v_perfil_nutricional_plano;

CREATE OR REPLACE VIEW v_perfil_nutricional_plano AS
...
SELECT
    ...
    COALESCE(p.skipped_fields, '{}'::jsonb) AS skipped_fields
FROM usuarios u
LEFT JOIN perfil_nutricional p ON p.usuario_id = u.id
LEFT JOIN peso_actual pw ON pw.perfil_nutricional_id = p.id
LEFT JOIN altura_actual ah ON ah.perfil_nutricional_id = p.id
LEFT JOIN enfermedades_agg enf ON enf.perfil_nutricional_id = p.id
LEFT JOIN restricciones_agg rs ON rs.perfil_nutricional_id = p.id
LEFT JOIN alergias_agg al ON al.perfil_nutricional_id = p.id
LEFT JOIN mae_patron_alimentario pa ON pa.id = p.patron_alimentario_id
LEFT JOIN mae_objetivo_nutricional obj ON obj.id = p.objetivo_nutricional_id
LEFT JOIN mae_distrito dist ON dist.id = p.distrito_id
LEFT JOIN mae_provincia prov ON prov.id = dist.provincia_id
LEFT JOIN mae_departamento dep ON dep.id = prov.departamento_id
LEFT JOIN extract_pivot ep ON ep.usuario_id = u.id;
